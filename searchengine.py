...
import sys
import string
# I do reuse my own indexes code from lab_1 with VP
from document import Article
from indexes import build_inverted_index, filter_index_references_by_all_terms
...
def create_index(filenames, index, file_titles):
    ...
    {'test1.txt': 'File 1 Title'}
    """
    articles = [Article.from_file_path(filename) for filename in filenames]
    inverted_index = build_inverted_index(articles)

    for filename, terms in inverted_index.items():
        index.setdefault(filename, []).extend(terms)

    for article in articles:
        file_titles[article.reference] = article.title
...
def search(index, query):
    ...
    return filter_index_references_by_all_terms(index, query.split())
